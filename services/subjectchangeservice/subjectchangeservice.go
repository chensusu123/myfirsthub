...
func SubjectChangeNotify(ctx context.Context, broadcastID string, userID int64) (err error) {
	...
	err = gNatsproducer.Publish(ctx, subject, []byte(broadcastID), natsproduceroption.WithSectionSubject(),
...
func Add(ctxP context.Context, broadcastID string, userID int64) (err error) {
	...
	err = SubjectChangeNotify(ctx, broadcastID, userID)
	if err != nil {
		return err
	}
	return nil
...
func Del(ctxP context.Context, broadcastID string, userID int64) (err error) {
	ctx := context.WithoutCancel(ctxP)
