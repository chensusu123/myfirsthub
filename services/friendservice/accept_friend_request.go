...
func (s *service) AcceptFriendRequestEvent(logger fklog.FKLogI, userId, fromId uint64) *errors.CodeError {
	// 好友列表
	friendModel, err := friendmodel.NewFriendModel(logger, userId)
	if err != nil {
...
// 删除好友请求事件
func (s *service) delSendFriendRequestEvent(logger fklog.FKLogI, userId, toId uint64) {
	// 补偿操作可能没必要
	friendModel, err := friendmodel.NewFriendModel(logger, userId)
	if err != nil {
