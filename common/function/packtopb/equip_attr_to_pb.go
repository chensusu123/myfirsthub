...
func EquipBaseAttrToCliPB(ctx context.Context, equipAttrs []*MazeEquipCache.BaseAttrInfo) (mainAttrs *MazeGameEquip.BaseAttrInfo, baseAttrs []*MazeGameEquip.BaseAttrInfo, attackScope, attackNumber, hurtType int32) {
	logger := fklog.ContextAppLogger(ctx)
	attackScopeAttrId := GetEquipAttackScopeCfg(ctx)
	attackNumberAttrId := GetEquipAttackNumberCfg(ctx)
	for _, attrInfoDb := range equipAttrs {
		...
		attrCfg := GMazeEquipAffixRandPoolV8Cfg.GetWithCtx(ctx, attrInfoDb.GetAttrGroup())
		if attrCfg != nil {
			baseAttrInfo := &MazeGameEquip.BaseAttrInfo{}
			for _, showAttrInfo := range attrInfoDb.ShowAttrList {
				attrInfo := EquipAttrToCliPb(ctx, showAttrInfo, attrCfg.Show_attr_min, attrCfg.Show_attr_max)
			...
			logger.CtxError(ctx, "maze_equip_affix_pool_v8【迷宫-装备-词条随机池】.xlsx 随机词条库缺失", zap.Any("池子id:", attrInfoDb.GetAttrGroup()))
		}
	}
	return
}
