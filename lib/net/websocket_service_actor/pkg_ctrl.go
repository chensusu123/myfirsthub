...
type SvrTCPEsRawProc func(ctx fknet.TCPContext, actorCtx actor.Context, es *raw_pkg.StruSvrEsRawBaseHead, data []byte) error

type ActorRawProc func(ctx fknet.TCPContext, actorCtx actor.Context, es *raw_pkg.StruSvrEsRawBaseHead, data []byte) error
...
func (ctl *SvrEsRawPackageCtrl) RegProc(packType uint16, pf ActorRawProc) error {
...
func (ctl *SvrEsRawPackageCtrl) RegJsonProc(packType uint16, pf ActorRawProc) error {
...
func (ctl *SvrEsRawPackageCtrl) IgnoreJsonProc(packType uint16) error {
	if _, load := ctl.jsonRoute.LoadOrStore(packType, ActorRawProc(func(ctx fknet.TCPContext, actorCtx actor.Context, es *raw_pkg.StruSvrEsRawBaseHead, data []byte) error {
...
func (ctl *SvrEsRawPackageCtrl) IgnoreProc(packType uint16) error {
	if _, load := ctl.route.LoadOrStore(packType, ActorRawProc(func(ctx fknet.TCPContext, actorCtx actor.Context, es *raw_pkg.StruSvrEsRawBaseHead, data []byte) error {
		ctx.DebugWF("ignore msg.", zap.Uint16("packType", packType))
		return nil
	})); load {
		return fmt.Errorf("packType %d is already registered or ignored", packType)
...
// Proc 处理tcp包
func (ctl *SvrEsRawPackageCtrl) ProcActor(ctx fknet.TCPContext, actorCtx actor.Context, data []byte) (err error) {
	// 防止异常中断
	...
	err = pf(ctx, actorCtx, &msg, msg.Data)
	if err != nil {
...
// Proc 处理tcp包
func (ctl *SvrEsRawPackageCtrl) ProcActorJson(ctx fknet.TCPContext, actorCtx actor.Context, data []byte) (err error) {
	// 防止异常中断
	defer fkalert.RecoverAlertException()
	//
	ctx.FKLogI.SetLogId(time.Now().UnixNano())
	...
	err = pf(ctx, actorCtx, &tcpPacket, data)
	if err != nil {
