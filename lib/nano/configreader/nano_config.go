...
var (
	...
	ErrConfigNotExist = errors.New("nano_actor/config: config not exist")

	// ErrProviderNotExist is provider not exist error
	ErrProviderNotExist = errors.New("nano_actor/config: provider not exist")

	// ErrCodecNotExist is codec not exist error
	ErrCodecNotExist = errors.New("nano_actor/config: codec not exist")
)
...
type LoadOption func(*NanoActorConfig)

// NanoActorConfigLoader is a config loader for trpc.
type NanoActorConfigLoader struct {
	watchers sync.Map
}

// Load returns the config specified by input parameter.
func (loader *NanoActorConfigLoader) Load(path string, opts ...LoadOption) (Config, error) {
	c, err := newNanoActorConfig(path, opts...)
	if err != nil {
		return nil, err
	}
...
func (loader *NanoActorConfigLoader) Reload(path string, opts ...LoadOption) error {
	c, err := newNanoActorConfig(path, opts...)
	if err != nil {
		return err
	}
...
func newNanoActorConfigLoad() *NanoActorConfigLoader {
	return &NanoActorConfigLoader{}
}

// DefaultConfigLoader is the default config loader.
var DefaultConfigLoader = newNanoActorConfigLoad()
...
type set struct {
	path  string
	mutex sync.RWMutex
	items []*NanoActorConfig
}

// get data
func (s *set) get(id string) *NanoActorConfig {
	s.mutex.RLock()
...
func (s *set) getOrStore(tc *NanoActorConfig) *NanoActorConfig {
...
func (s *set) watch(data []byte) {
	var items []*NanoActorConfig
	var del []*NanoActorConfig
	s.mutex.Lock()
	for _, v := range s.items {
		if v.watch {
...
var _ Config = (*NanoActorConfig)(nil)

// NanoActorConfig is used to parse yaml config file for trpc.
type NanoActorConfig struct {
...
func newNanoActorConfig(path string, opts ...LoadOption) (*NanoActorConfig, error) {
	c := &NanoActorConfig{
		path:    path,
		p:       GetProvider("file"),
...
func (c *NanoActorConfig) get() *entity {
	c.mutex.RLock()
...
func (c *NanoActorConfig) init() error {
	c.mutex.RLock()
	if c.value != nil {
		c.mutex.RUnlock()
		return nil
	...
	if err != nil {
		return fmt.Errorf("nano_actor/config failed to load error: %w config id: %s", err, c.id)
	}
	return c.set(data)
}

func (c *NanoActorConfig) doWatch(data []byte) error {
	c.mutex.Lock()
	defer c.mutex.Unlock()
	return c.set(data)
}

func (c *NanoActorConfig) set(data []byte) error {
	if c.expandEnv {
		data = ExpandEnv(data)
	}
	...
	if err != nil {
		return fmt.Errorf("nano_actor/config: failed to parse:%w, id:%s", err, c.id)
	}
	c.value = e
	return nil
}

func (c *NanoActorConfig) notify(data []byte, err error) {
	m := c.msg
...
// Load loads config.
func (c *NanoActorConfig) Load() error {
	if c.p == nil {
...
func (c *NanoActorConfig) Reload() {
	if err := c.Load(); err != nil {
		slog.Debug("nano_actor/config: failed to reload %s: %v", c.id, err)
	}
}

// Get returns config value by key. If key is absent will return the default value.
func (c *NanoActorConfig) Get(key string, defaultValue interface{}) interface{} {
...
func (c *NanoActorConfig) search(key string) (interface{}, bool) {
	e := c.get()
	...
	if err != nil {
		slog.Debug("trpc config: search key %s failed: %+v", key, err)
		return value, false
	}
