...
var ErrConfigNotSupport = errors.New("nano_actor/config: not support")
...
func GetWithUnmarshalProvider(key string, val interface{}, unmarshalName string, provider string) error {
	p := Get(provider)
	if p == nil {
		return fmt.Errorf("nano_actor/config: failed to get %s", provider)
	}
...
var unmarshalers = make(map[string]Unmarshaler)
...
var configMap = make(map[string]KVConfig)
