...
type MazeAttrListOrderV8Config struct {
	ConfigRows map[int32]*MazeAttrListOrderV8ConfigRow
	// lock       sync.RWMutex
}

// newConfig new one config
func newConfig() *MazeAttrListOrderV8Config {
	ret := &MazeAttrListOrderV8Config{ConfigRows: map[int32]*MazeAttrListOrderV8ConfigRow{}}
	return ret
}

// GetMazeAttrListOrderV8Config get one config by configId
func (c *MazeAttrListOrderV8Config) GetMazeAttrListOrderV8Config(configId int32) *MazeAttrListOrderV8ConfigRow {
	// c.lock.RLock()
...
func (c *MazeAttrListOrderV8Config) Get(configId int32) *MazeAttrListOrderV8ConfigRow {
	// c.lock.RLock()
	// defer c.lock.RUnlock()
	if cnf, ok := c.ConfigRows[configId]; ok {
		return cnf
...
func GetMazeAttrListOrderV8Config(configId int32) *MazeAttrListOrderV8ConfigRow {
	return gConfigData.GetMazeAttrListOrderV8Config(configId)
}

// Get pkg func. get one config by configId
func Get(configId int32) *MazeAttrListOrderV8ConfigRow {
	return GetWithCtx(context.Background(), configId)
}

// GetWithCtx pkg func. get one config by configId
func GetWithCtx(ctx context.Context, configId int32, otps ...config_manager.QueryOption) *MazeAttrListOrderV8ConfigRow {
	cfg := gConfigData.Get(configId)
	if cfg == nil {
		config_manager.MissRecord(ctx, "maze_attr_list_order_v8", configId, otps...)
	}
	return cfg
}
...
func GetAll() []*MazeAttrListOrderV8ConfigRow {
...
func SheetName() string {
	return "maze_attr_list_order_v8"
}
...
var cfgUpdateCallBack sync.Map //map[string]func(*MazeAttrListOrderV8Config)

// doConfigUpdateCallback do config update callback
func doConfigUpdateCallback(c *MazeAttrListOrderV8Config) {
	cfgUpdateCallBack.Range(func(key, value interface{}) bool {
		value.(func(*MazeAttrListOrderV8Config))(c)
		return true
	})
}

// RegisterMazeAttrListOrderV8InitCallBack reg config update func (old func)
var RegisterMazeAttrListOrderV8InitCallBack = RegisterLoadedCallBack

// RegisterLoadedCallBack reg config update func.
func RegisterLoadedCallBack(key string, f func(*MazeAttrListOrderV8Config)) {
	cfgUpdateCallBack.Store(key, f)
}

// data check call back
var cfgCheckCallBack sync.Map //map[string]func(*MazeAttrListOrderV8Config)error
...
func LoadDataManual(logger fklog.FKLogI, load func(file, sheet string, fields []string) ([][]string, error)) (err error) {
	...
	if len(data) < 1 {
		logger.WarnWF("load maze_attr_list_order_v8【迷宫-属性列表-排序】.xlsx maze_attr_list_order_v8 data empty.")
		return
	}
	container := loader.NewContainer()
	for k, row := range data {
		item := parser.New()
		if len(parser.Fields()) != len(gMazeAttrListOrderV8Fields) {
			err = errors.New("invalid request.fileds not match")
			logger.ErrorWF("parse maze_attr_list_order_v8【迷宫-属性列表-排序】.xlsx maze_attr_list_order_v8 failed.", zap.Int("row", k), zap.Strings("need", gMazeAttrListOrderV8Fields), zap.Strings("has", row))
			return
		}
		err = parser.Parse(logger, row, item)
		if err != nil {
			logger.ErrorWF("parse maze_attr_list_order_v8【迷宫-属性列表-排序】.xlsx maze_attr_list_order_v8 row data failed.", zap.Error(err))
			return
		}
		err = loader.Add(logger, container, item)
		if err != nil {
			logger.ErrorWF("add maze_attr_list_order_v8【迷宫-属性列表-排序】.xlsx maze_attr_list_order_v8 row data failed.", zap.Error(err))
			return
		}
	}
	err = loader.Check(container)
	if err != nil {
		logger.ErrorWF("check maze_attr_list_order_v8【迷宫-属性列表-排序】.xlsx maze_attr_list_order_v8 data failed.", zap.Error(err))
		return
	}
	loader.Swap(container)
	logger.InfoWF("load maze_attr_list_order_v8【迷宫-属性列表-排序】.xlsx maze_attr_list_order_v8 data success.")
	return
}
