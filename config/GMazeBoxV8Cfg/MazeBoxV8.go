...
type MazeBoxV8ConfigRow struct {
    ...
    Res_type_id res_type_id_first       int32  `json:"res_type_id res_type_id_first"` // 资源类型id
    Res_type_id_first       int32  `json:"res_type_id_first"` // 资源类型id（首次）
}
...
func (*gMazeBoxV8Parser) Parse(logger fklog.FKLogI,data []string, row interface{}) (err error) {
	...
	// parse column 10 res_type_first : 资源类型（首次） 
	if data[10] != "" {
	...
	// parse column 11 res_type_id res_type_id_first : 资源类型id 
	if data[11] != "" {
		tmp,err = strconv.ParseInt(data[11],10,64)
		if err != nil {
			err = errors.New("parse field res_type_id res_type_id_first 资源类型id to int32 failed")
			logger.ErrorWF("parse field res_type_id res_type_id_first 资源类型id to int32 failed.", 
				zap.String("xlsx", "maze_box_v8【迷宫-宝箱】.xlsx"), zap.String("sheet", "maze_box_v8"), 
				zap.String("parse_data",data[11]), 
				zap.Error(err))
			return
		}
		config.Res_type_id res_type_id_first = int32(tmp)
	}

	// parse column 12 res_type_id_first : 资源类型id（首次） 
	if data[12] != "" {
		tmp,err = strconv.ParseInt(data[12],10,64)
		if err != nil {
			err = errors.New("parse field res_type_id_first 资源类型id（首次） to int32 failed")
			logger.ErrorWF("parse field res_type_id_first 资源类型id（首次） to int32 failed.", 
				zap.String("xlsx", "maze_box_v8【迷宫-宝箱】.xlsx"), zap.String("sheet", "maze_box_v8"), 
				zap.String("parse_data",data[12]), 
				zap.Error(err))
			return
		}
		config.Res_type_id_first = int32(tmp)
	}
	return
}
