...
type MazeAnswerMechanismV8ConfigRow struct {
    ...
    Question bank_id       int32  `json:"question bank_id"` // 题库id
...
func (*gMazeAnswerMechanismV8Parser) Parse(logger fklog.FKLogI,data []string, row interface{}) (err error) {
	...
	// parse column 1 question bank_id : 题库id 
	if data[1] != "" {
		tmp,err = strconv.ParseInt(data[1],10,64)
		if err != nil {
			err = errors.New("parse field question bank_id 题库id to int32 failed")
			logger.ErrorWF("parse field question bank_id 题库id to int32 failed.", 
				zap.String("xlsx", "maze_answer_mechanism_v8【答题机关词库】.xlsx"), zap.String("sheet", "maze_answer_mechanism_v8"), 
				zap.String("parse_data",data[1]), 
				zap.Error(err))
			return
		}
		config.Question bank_id = int32(tmp)
	}
