...
func (*gMazeSkillInfoV8Parser) Parse(logger fklog.FKLogI, data []string, row interface{}) (err error) {
	...
	// parse column 13 scope_param1 : 技能效果半径
	if data[13] != "" {
	...
	// parse column 14 target_num : 目标数量
	if data[14] != "" {
	...
	// parse column 15 damage_element : 参与伤害计算的攻击元素类型（0-物理、1-冰、2-火、3-毒、4-电）
	if data[15] != "" {
	...
	// parse column 16 main_target_damage_fix : 主目标直接伤害固定值
	if data[16] != "" {
	...
	// parse column 17 main_target_damage : 主目标直接伤害系数
	if data[17] != "" {
	...
	// parse column 18 second_target_damage_fix : 非主目标直接伤害固定值
	if data[18] != "" {
	...
	// parse column 19 second_target_damage : 非主目标直接伤害系数
	if data[19] != "" {
	...
	// parse column 20 self_effect : 释放后自身效果
	if data[20] != "" {
	...
	// parse column 21 target_effect : 释放后对目标效果
	if data[21] != "" {
	...
	// parse column 22 target_effect_pro : 效果同组几率
	if data[22] != "" {
	...
	// parse column 23 is_allow : 允许释放状态
	if data[23] != "" {
	...
	// parse column 24 is_target : 能被选中的目标状态
	if data[24] != "" {
