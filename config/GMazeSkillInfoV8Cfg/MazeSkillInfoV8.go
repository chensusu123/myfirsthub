...
type MazeSkillInfoV8ConfigRow struct {
    ...
    Interval       map[int32]int32  `json:"interval"` // 技能伤害\效果生效间隔（毫秒）
}
...
func (*gMazeSkillInfoV8Parser) Parse(logger fklog.FKLogI,data []string, row interface{}) (err error) {
	...
	// parse column 29 interval : 技能伤害\效果生效间隔（毫秒） 
	if data[29] != "" {
		...
		for k,val := range vals {
			...
			tmp,err = strconv.ParseInt(items[0],10,64)
			if err != nil {
				err = errors.New("parse map field interval 技能伤害\效果生效间隔（毫秒） to key int32 failed")
				logger.ErrorWF("parse map field interval 技能伤害\效果生效间隔（毫秒） to key int32 failed.", 
					zap.String("xlsx", "maze_skill_info_v8【迷宫-技能-技能信息】.xlsx"), zap.String("sheet", "maze_skill_info_v8"), 
			...
			tmp,err = strconv.ParseInt(items[1],10,64)
			if err != nil {
				err = errors.New("parse map field interval 技能伤害\效果生效间隔（毫秒） to value int32 failed")
				logger.ErrorWF("parse map field interval 技能伤害\效果生效间隔（毫秒） to value int32 failed.", 
					zap.String("xlsx", "maze_skill_info_v8【迷宫-技能-技能信息】.xlsx"), zap.String("sheet", "maze_skill_info_v8"), 
					// zap.String("field_data",data[29]), 
					zap.String("item_data",val), zap.Int("index", k),
					zap.String("parse_data", items[1]),
