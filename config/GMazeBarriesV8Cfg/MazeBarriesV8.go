...
type MazeBarriesV8ConfigRow struct {
    ...
    Monster _max_num       map[int32]int32  `json:"monster _max_num"` // 区域刷怪数量上限
}
...
func (*gMazeBarriesV8Parser) Parse(logger fklog.FKLogI,data []string, row interface{}) (err error) {
	...
	// parse column 21 monster _max_num : 区域刷怪数量上限 
	if data[21] != "" {

		config.Monster _max_num = make(map[int32]int32)
		var key int32
		var value int32
		...
		for k,val := range vals {
			...
			tmp,err = strconv.ParseInt(items[0],10,64)
			if err != nil {
				err = errors.New("parse map field monster _max_num 区域刷怪数量上限 to key int32 failed")
				logger.ErrorWF("parse map field monster _max_num 区域刷怪数量上限 to key int32 failed.", 
					zap.String("xlsx", "maze_barries_v8【迷宫-关卡信息】.xlsx"), zap.String("sheet", "maze_barries_v8"), 
			...
			tmp,err = strconv.ParseInt(items[1],10,64)
			if err != nil {
				err = errors.New("parse map field monster _max_num 区域刷怪数量上限 to value int32 failed")
				logger.ErrorWF("parse map field monster _max_num 区域刷怪数量上限 to value int32 failed.", 
					zap.String("xlsx", "maze_barries_v8【迷宫-关卡信息】.xlsx"), zap.String("sheet", "maze_barries_v8"), 
					// zap.String("field_data",data[21]), 
					zap.String("item_data",val), zap.Int("index", k),
					zap.String("parse_data", items[1]),
			...
			value = int32(tmp)
			config.Monster _max_num[key] = value
		}
	}
	return
}
