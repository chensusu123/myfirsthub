...
func NewSendFriendRequestModel(logger fklog.FKLogI, userID uint64) (*SendFriendRequestModel, error) {
...
func (f *SendFriendRequestModel) load(logger fklog.FKLogI, userId uint64) (err error) {
	bytes, err := friendredis.GetSendFriendRequest(logger, userId)
	if err != nil {
		logger.ErrorWF("load send redis err", zap.Error(err))
		return nil
	}
	if len(bytes) == 0 {
	...
	if err != nil {
		logger.ErrorWF("load send unmarshal err", zap.Error(err), zap.Any("bytes", string(bytes)))
		return err
	}
