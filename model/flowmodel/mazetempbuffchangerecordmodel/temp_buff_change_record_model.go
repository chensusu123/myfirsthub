...
type MazeTempBuffChangeMsg struct {
	...
	NowBuffList  string `json:"now_buff_list" gorm:"now_buff_list"`   // 当前可以选择的buff列表
	SelectBuffID string `json:"select_buff_id" gorm:"select_buff_id"` // 选择的buffID
}

func NewMazeTempBuffChangeMsg(userID uint64, stageID int32, chgAttrsStr string, chgType int32, chgDesc, nowBuffList, selectBuffID string) *MazeTempBuffChangeMsg {
