...
func QueryMessages(logger fklog.FKLogI, appID int32, userID, peerID uint64, lastID uint64, limit int) (messages []Message, err error) {
	var (
...
func SaveMessage(logger fklog.FKLogI, appID int32, userID, peerID uint64, message Message) (err error) {
	var (
		key = getKey(appID, userID, peerID)
		ctx = context.Background()
	)
