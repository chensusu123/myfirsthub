...
func getKey(db nanoredis.NanoRedisClient, args ...interface{}) string {
	return db.MakeSectionKey(fmt.Sprintf("subscribe:group:alliance:u:%d", args...))
}
...
func SaveSubscribe(ctx context.Context, userID uint64, groupIDs []int64) error {
	...
	if err != nil {
		logger.CtxError(ctx, "alliance SaveSubscribe Client fail",
			zap.Error(err),
		)
		return err
	}
...
func GetSubscribe(ctx context.Context, userID uint64) ([]int64, error) {
	...
	cli, err := globalredis.GCli.GetDB()
	if err != nil {
		logger.CtxError(ctx, "alliance GetSubscribe Client fail",
			zap.Error(err),
		)
		return nil, err
	}
...
func DelSubscribe(ctx context.Context, userID uint64) error {
	...
	cli, err := globalredis.GCli.GetDB()
	if err != nil {
		logger.CtxError(ctx, "alliance DelSubscribe Client fail",
			zap.Error(err),
		)
		return err
	}
