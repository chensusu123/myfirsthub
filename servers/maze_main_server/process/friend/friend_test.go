...
func TestFriendList(t *testing.T) {
	friends, finish, codeErr := friendservice.GlobalFriendService.FriendList(context.TODO(), 40000001, 1, 20)
	if codeErr != nil {
		fmt.Println(codeErr)
		return
	}

	friendList := make([]*Friend.FriendInfo, 0)
	for _, friend := range friends {
		nowUserProfile, err := userprofileservice.GlobalUserProfileService.GetUserProfile(context.TODO(), friend.UserId)
		if err != nil {
			fmt.Println(err)
		}
		friendList = append(friendList, &Friend.FriendInfo{
