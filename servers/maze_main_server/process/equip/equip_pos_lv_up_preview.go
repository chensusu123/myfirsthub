...
func (e *Equip) OnEquipPosLvUpPreviewRQ_10423_10424(s *session.Session, rq *MazeEquipPos.MazeEquipPosLvUpPreviewRQ) (err error) {
	...
	if posId < 1 || posId > constdef.EquipPosNum {
		logger.CtxError(ctx, "OnEquipPosLvUpPreviewRQ param invalid", zap.Int32("posId", posId))
	...
	if len(equipPos) == 0 {
		logger.CtxError(ctx, "OnEquipPosLvUpPreviewRQ equip pos num is 0")
	...
	if curEquipPosDb == nil {
		logger.CtxError(ctx, "OnEquipPosLvUpPreviewRQ param invalid, no current equip position", zap.Int32("posId", posId))
		rs.ErrInfo = errors.ARGS_NOT_MATCH.ToInfo()
		return
	}
	curCfg := equipposexcel.GetPosStrengthCfg(posId, curLv)
	if curCfg == nil {
		logger.CtxError(ctx, "OnEquipPosLvUpPreviewRQ no found strength cfg",
