...
func (g *Frame) OnExistRoomRQ_10541_10542(s *session.Session, req *MazeRoom.MazeExistRoomRQ) (err error) {
	...
	logger.CtxInfo(ctx, "OnExistRoomRQ start", zap.Any("req", req))
	defer func() {
		err = s.Response(res)
		logger.CtxInfo(ctx, "OnExistRoomRQ end", zap.Any("res", res))
	}()
	...
	if err != nil {
		logger.CtxError(ctx, "OnExistRoomRQ GetUserInfoV2 fail", zap.Error(err))
		res.ErrInfo = errors.MODULE_ERROR.ToInfo()
		return
	}

	err = frame_service.ExistRoom(ctx, userId)
	if err != nil {
