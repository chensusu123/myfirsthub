...
func OnSubMazeEnergyRQ(ctx fkrpc.RPCContext, shardingID int64, rqMsg proto.Message, rsMsg proto.Message) (err error) {
	...
	if req.GetUserId() == 0 {
		userCtx.WarnWF("OnSubMazeEnergyRQ invalid userId ", zap.Any("rq", req))
		res.ErrInfo = errors.COMMON_ERROR_TIPS.Wrap("invalid userId")
		return nil
	}
	if req.GetTradeNumber() == 0 {
		userCtx.WarnWF("OnSubMazeEnergyRQ invalid tardeNo ", zap.Any("rq", req))
		res.ErrInfo = errors.COMMON_ERROR_TIPS.Wrap("invalid tardeNo")
		return nil
	}

	if req.GetOpType() == 0 {
		userCtx.WarnWF("OnSubMazeEnergyRQ invalid optype", zap.Any("rq", req))
		res.ErrInfo = errors.COMMON_ERROR_TIPS.Wrap("invalid optype")
		return nil
	}
	if req.GetSubVal() <= 0 {
		userCtx.WarnWF("OnSubMazeEnergyRQ invalid val", zap.Any("rq", req))
	...
	if err != nil {
		userCtx.ErrorWF("OnSubMazeEnergyRQ GetUserInfoV2 fail", zap.Error(err))
		res.ErrInfo = errors.MODULE_ERROR.ToInfo()
		return
	}
	if uInfo.Energy < req.GetSubVal() {
		userCtx.WarnWF("OnSubMazeEnergyRQ energy less",
	...
	if err != nil {
		userCtx.ErrorWF("OnSubMazeEnergyRQ SetUserInfoV2 fail", zap.Error(err), zap.Any("uInfo", uInfo))
		res.ErrInfo = errors.MODULE_ERROR.ToInfo()
		return
	}
	res.RemainVal = proto.Int32(remain)
	EndRecord(userCtx, record, 0-req.GetSubVal(), uInfo)
	return nil
}
